...
const advertisementImpressionSchema = new Schema<IAdvertisementImpression>({
  ...
  shownAt: { type: Date, default: Date.now, required: true, index: true },
  closedAt: { type: Date },
  clickedAt: { type: Date },
}, { timestamps: true });
...
export const AdvertisementImpression = mongoose.model<IAdvertisementImpression>('AdvertisementImpression', advertisementImpressionSchema);
