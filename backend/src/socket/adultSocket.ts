      ...
      try {
        ...
        try {
          ...
          if (err.code === 11000 || err.name === 'MongoServerError' || err.message?.includes('E11000') || err.message?.includes('duplicate key')) {
            const callerIdStr = socket.data.user._id.toString();
        ...
        adultNamespace.to(`user:${provider._id}`).emit('call:incoming', {
          ...
          receiverId: provider._id,
          receiverName: provider.providerProfile?.stageName || provider.displayName || provider.username,
          receiverAvatar: provider.profilePhoto || '/placeholder.svg',
          type: data.isVideo ? 'video' : 'audio',
