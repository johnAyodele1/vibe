...
export const fulfillTicketOrderInternal = async (orderId: string) => {
  ...
  const executeFulfillmentMutations = async (session?: mongoose.ClientSession) => {
    ...
    // 4. Pre-generate ticket codes and local fast QR Data URLs outside transaction network I/O
    const ticketsToCreate = [];
    for (let i = 0; i < qty; i++) {
      const ticketCode = await generateTicketCode();
      const qrData = `https://zippo.com.ng/ticket/${ticketCode}`;
      const encoded = encodeURIComponent(qrData);
      const localQrDataUrl = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400" viewBox="0 0 400 400"><rect width="100%" height="100%" fill="%23f5edf0"/><text x="50%" y="45%" dominant-baseline="middle" text-anchor="middle" font-family="monospace" font-size="16" font-weight="bold" fill="%23c8102e">ZIPPO TICKET QR</text><text x="50%" y="58%" dominant-baseline="middle" text-anchor="middle" font-family="monospace" font-size="14" font-weight="bold" fill="%230a0608">${encoded}</text></svg>`;
