...
if (process.env.REDIS_URL || process.env.REDIS_HOST) {
  try { redisClient = new Redis(process.env.REDIS_URL || ''); } catch (err) {}
}
...
export const getProviderPublicProfile = async (req: Request, res: Response) => {
  try {
    ...
    if (typeof providerId !== 'string' || !mongoose.Types.ObjectId.isValid(providerId)) {
      return res.status(400).json({ success: false, error: { code: 'INVALID_ID', message: 'Invalid provider ID format' } });
    }

    const provider = await AdultUser.findOne({ _id: providerId, role: 'provider' }).lean();
    if (!provider) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Provider not found' } });

    let profileViews = provider.providerProfile?.profileViews || 0;
    ...
    if (viewerId && viewerId.toString() !== providerId) {
      ...
      if (shouldRecordView) {
        const updatedProvider = await AdultUser.findByIdAndUpdate(
          ...
          { new: true, projection: { 'providerProfile.profileViews': 1 } }
        ).lean();
        profileViews = updatedProvider?.providerProfile?.profileViews ?? profileViews + 1;
    ...
    const photos = (provider.providerProfile?.photos || []).map((url, index) => ({ url, order: index, isExplicit: index > 0 }));
    ...
    if (memberId) {
      ...
      if (redisClient) {
        try {
          ...
          members.forEach(m => { const parsed = parseInt(m); if (!isNaN(parsed)) unlockedPhotoIndexes.push(parsed); });
        } catch (e) { console.error('smembers error:', e); }
      } else {
        const set = memoryPhotoUnlocks.get(unlockKey);
        if (set) set.forEach(m => { const parsed = parseInt(m); if (!isNaN(parsed)) unlockedPhotoIndexes.push(parsed); });
      }
    }

    const pricing = {
      perMinuteRate: provider.providerProfile?.servicesOffered?.includes('private_call') ? (provider.providerProfile?.pricePerMinute || null) : null,
      ...
      tonightRate: provider.providerProfile?.servicesOffered?.includes('hookup') ? (provider.providerProfile?.tonightRate || null) : null,
      tipMenu: (provider.providerProfile?.tipMenu || []).map(item => ({ amount: item.amount, description: item.action || 'Tip' }))
    };
    ...
    const publicProfile = {
      ...
      location: {
        ...
        country: { name: provider.providerProfile?.location?.country?.name || provider.location?.country?.name || 'Unknown' }
      },
...
export const getProviders = async (req: Request, res: Response) => {
  ...
  const query: any = { role: 'provider', status: 'active', 'providerProfile.onboarding.isComplete': true, isVerified: true };
  ...
  const [providers, total] = await Promise.all([
    AdultUser.find(query).select('providerProfile username displayName profilePhoto country createdAt').sort(sort).limit(Number(limit)).skip((Number(page) - 1) * Number(limit)).lean(),
...
export const applyAsProvider = async (req: Request, res: Response) => {
  ...
  user.role = 'provider';
  user.providerProfile = { stageName, idVerificationDocUrl, categories, contentTags, pricePerMinute: pricePerMinute || 0, tipMinimum: tipMinimum || 0, totalEarnings: 0, pendingPayout: 0, verificationStatus: 'pending', isLive: false, rating: { average: 0, count: 0 } };
  await user.save();
...
export const updateProviderProfile = async (req: Request, res: Response) => {
  const user = req.adultUser;
  if (!user || user.role !== 'provider') return res.status(403).json({ success: false, error: { code: 'FORBIDDEN', message: 'Only providers can update profile' } });
  const { stageName, bio, country, profilePhoto, categories, contentTags, pricePerMinute, tipMinimum, videoCallPrice, audioCallPrice, privateSextPrice } = req.body;
  ...
  if (!user.providerProfile) {
    user.providerProfile = { stageName: stageName || '', categories: categories || [], contentTags: contentTags || [], pricePerMinute: pricePerMinute || 0, tipMinimum: tipMinimum || 0, videoCallPrice: videoCallPrice || 0, audioCallPrice: audioCallPrice || 0, privateSextPrice: privateSextPrice || 0, totalEarnings: 0, pendingPayout: 0, verificationStatus: 'pending', isLive: false, rating: { average: 0, count: 0 } };
  } else {
  ...
  await user.save();
  res.json({ success: true, message: 'Profile updated successfully', data: { user: { id: user._id, bio: user.bio, country: user.country, profilePhoto: user.profilePhoto, providerProfile: user.providerProfile } } });
};
...
export const unlockProviderPhoto = async (req: Request, res: Response) => {
  try {
    ...
    if (!memberId) return res.status(401).json({ success: false, error: { code: 'UNAUTHORIZED', message: 'Authentication required' } });
    const UNLOCK_COST = 1;
    const provider = await AdultUser.findOne({ _id: providerId, role: 'provider' });
    if (!provider || !provider.providerProfile || !provider.providerProfile.photos) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Provider profile not found' } });
    const idx = parseInt(photoIndex);
    const photosCount = provider.providerProfile.photos.length;
    if (isNaN(idx) || idx < 1 || idx >= photosCount) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Photo not found' } });
    const realPhotoUrl = provider.providerProfile.photos[idx];
    const unlockKey = `unlock:provider:${providerId}:member:${memberId}`;
    let alreadyUnlocked = false;
    if (redisClient) {
      try { alreadyUnlocked = (await redisClient.sismember(unlockKey, idx.toString())) === 1; } catch (err) { console.error('Redis sismember error:', err); }
    } else alreadyUnlocked = !!memoryPhotoUnlocks.get(unlockKey)?.has(idx.toString());
    if (alreadyUnlocked) return res.json({ success: true, url: realPhotoUrl, alreadyUnlocked: true });
    const memberCredits = req.adultUser!.credits;
    if (memberCredits < UNLOCK_COST) return res.status(402).json({ success: false, error: { code: 'INSUFFICIENT_FUNDS', message: 'Not enough credits', required: UNLOCK_COST } });
    const updatedMember = await AdultUser.findOneAndUpdate({ _id: memberId, credits: { $gte: UNLOCK_COST } }, { $inc: { credits: -UNLOCK_COST } }, { new: true });
    if (!updatedMember) return res.status(402).json({ success: false, error: { code: 'INSUFFICIENT_FUNDS', message: 'Not enough credits', required: UNLOCK_COST } });
    const updatedProvider = await AdultUser.findByIdAndUpdate(providerId, { $inc: { credits: UNLOCK_COST, 'providerProfile.totalEarnings': UNLOCK_COST } }, { new: true });
    await CreditTransaction.create({ userId: memberId, type: 'paid_media_unlock', amount: UNLOCK_COST, usdAmount: 0, description: `Unlocked photo index ${idx} for provider ${provider.providerProfile?.stageName || provider.displayName}`, relatedUserId: provider._id, status: 'completed', platformFee: 0, eligibleForPayout: true });
    await CreditTransaction.create({ userId: provider._id, type: 'paid_media_unlock', amount: UNLOCK_COST, usdAmount: 0, description: `Earning from photo index ${idx} unlocked by member ${updatedMember.displayName}`, relatedUserId: memberId, status: 'completed', platformFee: 0, eligibleForPayout: true });
    if (redisClient) {
      try { await redisClient.sadd(unlockKey, idx.toString()); await redisClient.expire(unlockKey, 30 * 24 * 60 * 60); } catch (err) { console.error('Redis sadd error:', err); }
    } else {
      if (!memoryPhotoUnlocks.has(unlockKey)) memoryPhotoUnlocks.set(unlockKey, new Set());
    ...
    if (updatedProvider) dynamicSocketService.emitToUser(providerId.toString(), 'wallet:updated', { balance: updatedProvider.credits });
