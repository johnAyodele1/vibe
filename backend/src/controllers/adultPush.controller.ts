    ...
    for (const sub of subscriptions) {
      ...
      try {
        ...
        if (err.statusCode === 410 || err.statusCode === 404) {
          try {
            await PushSubscription.findOneAndUpdate({ _id: sub._id }, { $set: { isActive: false, notificationsEnabled: false, deactivatedAt: new Date(), pushHealthStatus: 'unhealthy' }, $unset: { endpoint: 1, keys: 1 } });
          } catch (cleanErr: any) {
            console.error('[Push][Test] Cleanup failed safely:', cleanErr.message);
          }
        } else if (err.statusCode === 403) {
          try {
            await PushSubscription.deleteOne({ _id: sub._id });
          } catch (cleanErr: any) {
            console.error('[Push][Test] Delete sub failed safely:', cleanErr.message);
          }
        }
      }
    }
