...
self.addEventListener('activate', event => {
  event.waitUntil(caches.keys().then(names => Promise.all(names.filter(name => name !== CACHE_NAME).map(name => caches.delete(name)))).then(() => self.clients.claim()).then(() => self.clients.matchAll({ type: 'window' })).then(clients => clients.forEach(client => client.postMessage({ type: 'SW_UPDATED', version: SW_VERSION }))));
});
self.addEventListener('fetch', event => {
  ...
  if (event.request.headers.get('accept')?.includes('text/html')) event.respondWith(fetch(event.request).then(response => { if (response.ok) caches.open(CACHE_NAME).then(cache => cache.put(event.request, response.clone())); return response; }).catch(async () => (await caches.match(event.request)) || (await caches.match('/')) || caches.match('/offline.html')));
});
...
self.addEventListener('message', event => {
  if (event.data?.type === 'UPDATE_BADGE') { const count = event.data.count; if (count > 0 && navigator.setAppBadge) navigator.setAppBadge(count).catch(() => {}); if (!count && navigator.clearAppBadge) navigator.clearAppBadge().catch(() => {}); }
});
const urlBase64ToUint8Array = base64String => { const padding = '='.repeat((4 - base64String.length % 4) % 4); const raw = atob((base64String + padding).replace(/-/g, '+').replace(/_/g, '/')); return new Uint8Array(Array.from(raw).map(char => char.charCodeAt(0))); };
const fetchVapidKeyInSW = async () => { try { const response = await fetch(`${API_BASE_URL}/v1/adult/push/public-key`); if (!response.ok) return null; const data = await response.json(); return data.publicKey || null; } catch { return null; } };
self.addEventListener('pushsubscriptionchange', event => {
  event.waitUntil((async () => {
    ...
    } catch (error) { console.error('[SW] pushsubscriptionchange failed:', error); }
  })());
});
